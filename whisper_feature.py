...
def process_whisper_single_line(line, oss_client, model, device, output_dir, rank, feat_index):
    """单条文件路径：加载 16kHz 音频，用 Whisper 提取特征，保存 audio_features 到 .npy，返回带路径的 JSON 行。"""
    try:
        audio_path = line
        ...
        # Convert torch tensor to numpy array (whisper expects numpy)
        audio_np = signals.squeeze(0).numpy()
        dur = round(len(audio_np) / 16000, 2)
        ...
        with torch.no_grad():
            ...
            audio_features = whisper.DecodingTask(model, options)._get_audio_features(mel)
            feats = audio_features.cpu()
        
        feats_np = feats.numpy()
        feats_dir = os.path.join(output_dir, "whisper_feats")
        os.makedirs(feats_dir, exist_ok=True)
        feat_path = os.path.join(feats_dir, f"feat_{rank}_{feat_index}.npy")
        np.save(feat_path, feats_np)
        jsd = {
            "audio_filepath": audio_path,
            "whisper_feature_path": feat_path,
            "whisper_feature_shape": list(feats_np.shape),
            "duration": dur
        }
        return json.dumps(jsd, ensure_ascii=False) + "\n"
    except Exception as e:
        print(f"process_whisper_single_line error: {e}")
        traceback.print_exc()
        return None


def process_whisper_feature(lines, save_file, output_dir, device, rank=0, model_name="large-v3"):
    """批量用 Whisper 提取特征：加载模型一次，逐条处理并写入 save_file，特征存 output_dir/whisper_feats/*.npy。"""
    model = whisper.load_model(model_name)
    model = model.to(device).eval()
    print("Whisper model loaded for feature extraction.")
    results = []
    for idx, line in enumerate(tqdm(lines)):
        oss_client = ossb_pool.get_conn()
        ret = process_whisper_single_line(line, oss_client, model, device, output_dir, rank, idx)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model_name", type=str, default="large-v3")
    ...
    process_whisper_feature(
        cut_lines,
        save_file=f"{output_dir}/music_whisper_{world_size}_{rank}.jsonl",
